function [modFloor,roomData] = GenerateFloor(floor, roomSize)
...
for iRow=1:floorSize(1)
    for iCol=1:floorSize(2)
        ...
        if roomData(iRow, iCol).Type ~= 0
            ...
            elseif iRow < roomSize(1) - 1 %check above for any row other than the last one
            ...
            roomData(iRow, iCol).TileInfo.Theme.color = randi(13, 'uint8');
            ...
            roomData(iRow, iCol).TileInfo.Theme.obstacle = randi(3, 'uint8');
            
            %third value determines if water will be used instead of lava, it is
            %a logical variable
            roomData(iRow, iCol).TileInfo.Theme.lava = logical(round(rand(1)));
            ...
            %will use pyramids
            roomData(iRow, iCol).TileInfo.Theme.block = logical(round(rand(1)));
